...
var getLookUps = function () {
    ...
    var allDict = parse(program.alld) // {page:{other:cWrites:n}, js:{lookups:0, cSize:0, sSize:0, cWrites:0}}
    ...
    pages.forEach((page) => {
        ...
        var allCWrites = allDict[page].other.cWrites;
        var jsAllData = allDict[page].js;
        var allSData = otherServe(page);

        var pageData = {
            lookups: { orig: jsAllData.lookups + allSData.lookups, union: jsData.lookups.union + allSData.lookups },
            cSize: { orig: allCWrites * CRAWL_ENTRY_LEN + jsAllData.cSize, union: allCWrites * CRAWL_ENTRY_LEN + jsData.cSize.union },
            sSize: { orig: allSData.sSize + jsAllData.sSize, union: allSData.sSize + jsData.sSize.union },
            cWrites: { orig: jsAllData.cWrites + allCWrites, union: jsData.cWrites.union + allCWrites }
        }
