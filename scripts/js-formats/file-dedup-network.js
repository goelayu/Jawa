...
var updateDataStore = function (n, dataStore, engines, page) {
    var localStore = {
    ...
    for (var n of net) {
        ...
        if (dataStore[type].keys.indexOf(key) < 0) {
            ...
            //     !filter_adblk && (dataStore[type].filter_adblk += n.size) && (localStore[type].filter_adblk += n.size);
            //     !filter_custom && (dataStore[type].filter_custom += n.size) && (localStore[type].filter_custom += n.size);
            //     (!filter_custom && !filter_adblk) && (dataStore[type].filter_both += n.size) && (localStore[type].filter_both += n.size);
            // }
            // if (type == 'js') {
            //     console.log(`url: ${url}, size: ${n.size}`);
            //     njsFiles++;
            // }
        ...
        localStore[type].total += n.size;
    }
    program.verbose && console.log(`${page} nFiles: ${njsFiles}`)
    program.verbose && console.log(localStore.other.total, localStore.other.dedup, localStore.js.total, localStore.js.dedup);
}
...
var main = function () {
    ...
    pages.split('\n').forEach((p) => {
        ...
        try {
            ...
            updateDataStore(p, dataStore, [engineADBL, engineCUSTOM], p);
    ...
    console.log(dataStore.other.total, dataStore.other.dedup, dataStore.js.total, dataStore.js.dedup);
}
