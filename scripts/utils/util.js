...
var pruneDB = function (d) {
  ...
  d.forEach((entry) => {
    if (entry.category.indexOf(critera) >= 0) console.log(entry.url);
  });
};
...
var getQueueTime = function (data) {
  ...
  for (var n of net) {
    ...
    if (n.redirectResponse) {
      console.log(
        (n.redirectResponse.timing.requestTime - n.requestStart_o) * 1000
      );
...
var getStallTime = function (data) {
  ...
  for (var n of net) {
    ...
    if (n.redirectResponse) {
      console.log(
        n.redirectResponse.timing.sendStart - n.redirectResponse.timing.sslEnd
      );
    }
    if (n.response)
...
var getSizePerType = function (data) {
  ...
  for (var n of net) {
    ...
    //     type = 'Other';
    if (!type) type = "other";
    if (!(type in type2size)) type2size[type] = 0;
    type2size[type] += n.size;
  }
  var totalSize = Object.values(type2size).reduce((acc, cur) => {
    return acc + cur;
  }, 0);
...
var getNumPerType = function (data) {
  ...
  for (var n of net) {
    ...
    if (!(n.type in type2num)) type2num[n.type] = 0;
    type2num[n.type] += 1 + n.redirects.length;
  }
  var totalSize = Object.values(type2num).reduce((acc, cur) => {
    return acc + cur;
  }, 0);
  // Object.keys(type2size).forEach((t)=>{
...
var estimateSPTime = function (data) {
  ...
  frames.forEach((f) => {
    ...
    if (f == baseFrame)
      ...
      plt = Math.max(
        plt,
        frameOrigTime[baseFrame] + frameOrigTime[f] + frameMaxResTime[f]
      );
  });
  console.log(plt);
};
...
var filler = function (data) {
  ...
  var net = netParser.parseNetworkLogs(parse(data));
  for (var n of net) {
    if (!n.response) console.log(n.url);
  }
};
...
var findMatchForFailed = function (net1, net2) {
  ...
  var getOrigUrl = function (n) {
    ...
    if (_origUrl.length > 1) return _origUrl[1];
  };
...
var failedRequestsBreakdown = function (net1, net2) {
  ...
  for (var f of failedRequests) {
    ...
    var fuzzBestMatch = fuzzMatch(
      f.url,
      net1.filter((e) => e.type == f.type).map((e) => e.url)
    );
...
var getResOnPage = function (data) {
  var net = netParser.parseNetworkLogs(parse(data)),
    ...
    size = 0;
  var urlSeen = [],
    typePrefix = ["js_", "im_", "cs_", "if_"];
  for (var n of net) {
    if (ignoreUrl(n, net)) {
      continue;
    }
    if (program.siteType == "archive") {
      if (!isCriticalReq(n)) continue;
      var _oUrl = n.url;
      for (var t of typePrefix) {
        _oUrl = _oUrl.replace(t, "");
      }
      if (urlSeen.indexOf(_oUrl) >= 0) continue;
      urlSeen.push(_oUrl);
    }
...
var isInitiatedBy = function (src, target) {
  if (target.initiator && target.initiator.type == "script") {
    for (var cs of target.initiator.stack.callFrames) {
      if (cs.url == src || cs.url.split("?")[0] == src.split("?")[0])
        return true;
    }
  }
  return false;
...
var initiatedRequests = function (allNet, filterNet) {
  ...
  let easyListTxt = fs.readFileSync(
    "/vault-home/goelayu/webArchive/filter-lists/final.txt",
    "utf-8"
  );
  ...
  var resInitiated = {};
  var unfilteredScripts = filterNet.filter(
    (e) => !ignoreUrl(e) && e.type.toLowerCase().indexOf("script") >= 0
  );
  ...
  for (var a of allNet) {
    ...
    if (
      a.url.indexOf("amazon") >= 0 ||
      a.url.indexOf("beacon") >= 0 ||
      a.url.indexOf("ntv") >= 0
    )
      continue;
    ...
    if (
      ABPFilterParser.matches(parsedFilterData, a.url, {
        domain: domain,
      })
    )
      continue;
  ...
  var allInit = Object.values(resInitiated).reduce((acc, cur) => {
      return cur.all.length + acc;
    }, 0),
    filInit = Object.values(resInitiated).reduce((acc, cur) => {
      return cur.filtered.length + acc;
    }, 0);
...
var _combineCoverage = function (cvgArr, urlCoverage, urlToRanges) {
  ...
  cvgArr.forEach((coverage) => {
    for (var entry of coverage) {
      if (
        entry.url.indexOf(".js") > 0 &&
        entry.url.indexOf("chromewebdata") < 0
      ) {
...
switch (program.type) {
  case "prune":
    ...
    break;
  case "netSize":
    getNetSize(parse(program.input));
    break;
  case "netLen":
    getNumPerType(parse(program.input));
    break;
  case "stall":
    getStallTime(program.input);
    break;
  case "queue":
    getQueueTime(program.input);
    break;
  case "len":
    getNetLen(program.input);
    break;
  case "discovery":
    discoverTime(program.input);
    break;
  case "sp":
    estimateSPTime(program.input);
    break;
  case "st":
    getSizePerType(program.input);
    break;
  case "ss":
    estimateSnapshotSize(program.input);
    break;
  case "res":
    getResOnPage(program.input);
    break;
  case "fil":
  ...
  case "sum":
    console.log(
      Object.values(JSON.parse(fs.readFileSync(program.input, "utf-8"))).reduce(
        (acc, cur) => {
          return acc + cur;
        },
        0
  ...
  case "initiator":
    ...
    break;
  case "coverage":
    combineCoverage(program.input, program.anotherin);
    break;
  case "Icoverage":
    instCoverage(program.input, program.anotherin);
    break;
  case "quick":
    quickNet(parse(program.input));
    break;
  case "brk":
    failedRequestsBreakdown(program.input, program.anotherin);
    break;
  case "failedB":
    findMatchForFailed(program.input, program.anotherin);
    break;
  case "evtfn":
    singleFnEvt(program.input);
    break;
}
