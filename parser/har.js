...
function parsePage(pageId, stats) {
    ...
    return {
        page: {
            ...
            startedDateTime,
            pageTimings: {
                'onContentLoad':stats.domContentLoaded,
                'onLoad':stats.onLoad
            },
            _user: stats.user
        },
...
function parseEntry(pageref, entry) {
    ...
    //hack because frommemory cache is not captured, only from disk cache is captured
    if (payload.response.transferSize == 0)
        timings.receive = .001;
...
function computeTimings(entry) {
    ...
    return {
        time,
        timings: {blocked, dns, connect, send, wait, receive, ssl, _blocked_queueing}
    };
}
