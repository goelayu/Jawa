...
while read crawler; do 
    ...
    cd $proc_dir
    #  add separate cg directory
    mkdir -p $proc_dir/$crawler/evt/
    tar -xf ${src_dir}/${crawler}/${crawler}.performance.tar
    tar -xf ${src_dir}/${crawler}/${crawler}.instOutput.tar

    cd $proc_dir/$crawler/evt/
    tar -xf ${src_dir}/${crawler}/${crawler}.performance.evt.tar
    # find . -iname allFns | cut -d/ -f4,5,6 > ${fn_dir}/$crawler
    cd $script_dir
    # node all-mime-storage.js -v -d $proc_dir/$crawler/instOutput -u <(cat ${fn_dir}/$crawler) -o $result_dir/storage/pageMD/${crawler}.other &> $result_dir/storage/all_resources/$crawler &
    node --max-old-space-size=100000 js-formats/fn-cmp.js -v -d $proc_dir/$crawler/instOutput/ -u <(cat ${fn_dir}/$crawler | sort -t/ -nk2  ) \
    -v -p $proc_dir/$crawler/performance/ -e $proc_dir/$crawler/evt/$crawler/performance/ -o $result_dir/storage/pageMD/$crawler &> $result_dir/storage/js/resources/$crawler.evt.missing
    ...
    # node js-formats/file-dedup-network.js -v -n <(cat ${fn_dir}/$crawler | awk -v crawler=$crawler '{print "/w/goelayu/webArchive/data/"crawler"/performance/"$0"/network"}') &> $result_dir/storage/all_resources/$crawler.network
