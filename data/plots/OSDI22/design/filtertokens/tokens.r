...
# data$network <- factor(data$network, levels=c('IA', 'Querystrip', 'Fuzzy'))
...
ggplot(data, aes(x=x, y=y)) +
	...
	ylab(expression(atop(" # of filter tokens", paste("")))) + 
	guides(fill=guide_legend(title="Config")) +
	coord_cartesian(ylim=c(0, 50),xlim=c(0,190)) +
	scale_x_continuous(expand=c(0, 0),breaks=c(0,50,100,150,170)) +
