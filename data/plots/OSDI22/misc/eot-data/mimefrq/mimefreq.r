...
data$network <- factor(data$network, levels=c('text/html', 'application/javascript'))
pdf(output, height=2.5, width=5.0)
ggplot(data, aes(x=plt, linetype=network)) +
	...
	ylab(expression(atop("CDF across crawls", paste("")))) + 
	...
	guides(colour=guide_legend(nrow=1, byrow=TRUE), linetype=guide_legend(nrow=1, byrow=TRUE), direction="vertical") +
	theme(
