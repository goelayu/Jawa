...
# data$network <- factor(data$network, levels=c('Script','Image','Other','Document','Stylesheet'))
pdf(output, height=2.5, width=5.0)
# png(output, height=2.5, width=5.0)
ggplot(data, aes(x=rn,y=ratio)) +
	...
	ylab(expression(atop("JS bytes/All bytes"))) + 
	guides(fill=guide_legend(title="Config")) +
	coord_cartesian(ylim=c(0, 1),xlim=c(0,8000)) +
